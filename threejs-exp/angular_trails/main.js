 ...
 */
(function() {
  var HALF_PI, camera, clicked, ctrls, d, e, emitter, emitters, getEmitter, getLineColors, getLineMat, getLineVerts, getVertices, getWireMat, gui, head_geo, hideInfoPanel, info_panel, log, mouse, mouse_mesh, num_emitters, num_verts, onKeyUp, onMouseMove, panel, radius, rand_angle, renderFrame, renderer, scene, showInfoPanel, tetra_geo, toggleFollow, toggleFreeze, toggleInfoPanel, w, windowHalf;
  d = document;
  w = window;
  windowHalf = {
    x: w.innerWidth * 0.5,
  ...
  w.ctrls = ctrls;
  gui = new dat.GUI();
  gui.add(ctrls, 'auto_clear').name('Auto Clear');
  gui.add(ctrls, 'use_rainbow').name('Rainbow Mode');
  ...
  emitters = [];
  getEmitter = function(pos, base_hue) {
    ...
    line_anim = function() {
      ...
      while (i > 0) {
        ...
        this.line_geo.colors[i].setHSL(hue, sat, (1.0 - i / num_verts) * 0.35 + 0.15);
        i -= 1;
      }
